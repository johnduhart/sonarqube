...
public class IssueMetricFormulaFactoryImpl implements IssueMetricFormulaFactory {
  ...
  private static final List<IssueMetricFormula> FORMULAS = asList(
    ...
    new IssueMetricFormula(CoreMetrics.SQALE_DEBT_RATIO, false,
      (context, issues) -> context.setValue(100.0 * debtDensity(context)), asList(CoreMetrics.TECHNICAL_DEBT, CoreMetrics.DEVELOPMENT_COST)),
    ...
    new IssueMetricFormula(CoreMetrics.NEW_RELIABILITY_RATING, true,
      (context, issues) -> context.setValue(issues.getHighestSeverityOfUnresolved(RuleType.BUG, true).orElse(Severity.INFO))),

    new IssueMetricFormula(CoreMetrics.NEW_SECURITY_RATING, true,
      (context, issues) -> context.setValue(issues.getHighestSeverityOfUnresolved(RuleType.VULNERABILITY, true).orElse(Severity.INFO)))
  );
  ...
  private static double debtDensity(IssueMetricFormula.Context context) {
    double debt = context.getValue(CoreMetrics.TECHNICAL_DEBT).orElse(0.0);
  ...
  // TODO new_maintainability_rating, new_sqale_debt_ratio
